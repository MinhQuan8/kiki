...
const config = {
    name: "cuddle",
    description: "Lệnh âu yếm người khác bằng cách tag tên.",
    type: "entertain",
    usage: "/kiki cuddle [ TAG ]",
    condition: ["cuddle", "cudle"],
    exception: [],
    permission: 0,
    priority: 0
}
...
async function onCall({ message }) {
    ...
    const errorSentences = [
        ...
        "Lỗi r , có cái cuddle cx đ xog 🙂"
    ];
    const notFoundSentences = [
        "M cuddle cai lồn j v 🙂",
        "M đ bt dùng lệnh à? Cuddle ai??",
        "M cuddle ông nội t chắc? cuddle th nao? 🙂",
        "Cuddle không khí chắc?, cak j v 🙂",
    ];
    ...
    try {
        ...
        const getCuddle = async () => {
            for (let i = 0; i < targetID.length; i++) {
                const imageUrl = await aflb.sfw.cuddle();
