...
const config = {
    name: "slap",
    description: "Lệnh tát người khác bằng cách tag tên.",
    type: "entertain",
    usage: "/kiki slap [ TAG ]",
    condition: ["slap",],
    exception: [],
...
async function onCall({ message }) {
    ...
    const errorSentences = [
        ...
        "Lỗi r , có cái slap cx đ xog 🙂"
    ];
    const notFoundSentences = [
        "M slap cai lồn j v 🙂",
        "M đ bt dùng lệnh à? Tát ai??",
        "M tán ông nội m chắc? Tát th nao? 🙂",
        "Slap không khí chắc?, cak j v 🙂",
    ];
    ...
    try {
        ...
        const getSlap = async () => {
            for (let i = 0; i < targetID.length; i++) {
                const imageUrl = random(0, 1) ? await aflb.sfw.slap() : random(0, 1) ? await aflb.sfw.slapAss() : await aflb.sfw.slapCheek();
